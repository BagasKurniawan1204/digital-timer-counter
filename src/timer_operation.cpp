...
// ISR for frequency measurement timer (1s interval)
bool IRAM_ATTR timer_isr_freq_handler(void *arg) {
    ...
    s_ch1_frequency_hz = (current_count - s_ch1_last_count); // 1s window = frequency in Hz
    ...
    s_ch2_frequency_hz = (current_count - s_ch2_last_count); // 1s window = frequency in Hz
